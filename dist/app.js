...
import {stages,pages,missingItems,progressCounts} from './reporting.js';
import {installComfort} from './comfort.js';
installComfort();
const root=document.querySelector('main');
const KEY='diagnostic-participant-v1',DEMO_KEY='diagnostic-demo-v1',PENDING_KEY='diagnostic-inscription-v1';
let state=null,error='',storageWarning='',savingFor=null,saveTimer;
function restored(key){const s=localRead(key);if(!s||typeof s.id!=='string'||typeof s.name!=='string'||!s.answers||typeof s.answers!=='object'||Array.isArray(s.answers))return null;s.position=Math.max(0,Math.min(pages.length-1,Number(s.position)||0));s.revision=Number(s.revision)||0;return s;}
const val=k=>esc(state?.answers?.[k]??'');
const pending=()=>state&&!state.demo&&state.savedRevision!==state.revision;
...
function persist(target=state){
 if(!target)return;
 if(!localWrite(target.demo?DEMO_KEY:KEY,target))storageWarning='La copie locale est indisponible. Gardez cette page ouverte et téléchargez une copie avant de partir.';
}
function changed(){state.revision++;persist();clearTimeout(saveTimer);saveTimer=setTimeout(sync,900);status();}
function status(){
...
async function sync(){
 if(!state||state.demo||savingFor||!pending())return;
...
function welcome(){
 const previous=restored(KEY),trial=restored(DEMO_KEY);const active=configured();
 root.innerHTML=`<div class="intro"><p class="eyebrow">Diagnostic de départ · 1 h 30</p><h1>Vos usages du numérique</h1><p class="lead">Des questions et des exercices pour préparer des ateliers adaptés à vos besoins.</p>${!active?'<div class="notice"><strong>La version de découverte est disponible.</strong><br>La collecte réelle sera activée après configuration par l’animateur.</div>':''}${previous?`<section class="card resume-card"><p class="eyebrow">Parcours retrouvé sur ce poste</p><h2>${esc(previous.name)} · ${esc(previous.code)}</h2><p>${previous.finished?'Parcours terminé':'Étape '+(previous.position+1)+' sur '+pages.length}${previous.savedRevision!==previous.revision?' · Des réponses restent à transmettre.':''}</p><div class="actions"><button id="resume">Reprendre mon parcours</button><button class="secondary" id="forget">Effacer la copie locale</button></div></section>`:''}<form class="card" id="join"><h2>${active?'Rejoindre ma séance':'Découvrir le parcours'}</h2><div class="form-row"><div><label for="name">Prénom ou pseudonyme</label><input id="name" name="name" maxlength="60" autocomplete="given-name" placeholder="Ex. Camille" ${active?'required':''}></div>${active?'<div><label for="code">Code de séance</label><input id="code" name="code" maxlength="16" autocapitalize="characters" spellcheck="false" autocomplete="off" placeholder="Donné par votre animateur" required></div>':''}</div>${active?'<p class="muted">Vos réponses sont accessibles à votre animateur pour préparer les ateliers. Vous n’avez pas de compte à créer.</p>':'<p class="muted">Essayez librement : aucune réponse ne sera envoyée à un formateur.</p>'}<p id="join-error" role="alert"></p><div class="actions">${active?'<button type="submit">Rejoindre la séance</button>':''}<button type="button" class="${active?'secondary':''}" id="demo">Essayer la démonstration</button>${trial?'<button type="button" class="secondary" id="resume-demo">Reprendre mon essai</button>':''}</div></form><div class="welcome-points"><p><strong>À votre rythme</strong><br>Vous pouvez passer une question et y revenir.</p><p><strong>Avec votre animateur</strong><br>Les manipulations se font sur l’ordinateur.</p><p><strong>Sans classement</strong><br>« Je ne sais pas » est une réponse utile.</p></div></div>`;
 ...
 document.querySelector('#forget')?.addEventListener('click',()=>{if(confirm('Effacer uniquement la copie sur ce poste ? Les réponses déjà transmises resteront chez votre animateur.')){try{localStorage.removeItem(KEY);}catch{}state=null;welcome();}});
 document.querySelector('#join').onsubmit=async e=>{
  e.preventDefault();if(!active){document.querySelector('#demo').click();return;}
  const button=e.submitter;button.disabled=true;button.textContent='Connexion…';
  const f=new FormData(e.target),name=f.get('name').trim(),code=f.get('code').replace(/\s/g,'').toUpperCase();
  try{
   ...
   await rpc('join_session',{p_code:code,p_name:name,p_id:next.id,p_secret:next.secret});
   start(name,code,false,next);try{localStorage.removeItem(PENDING_KEY);}catch{}render(true);changed();
...
function start(name,code,demo,ids={id:crypto.randomUUID(),secret:''}){
 state={...ids,name,code,demo,position:0,answers:{},revision:0,savedRevision:-1,startedAt:new Date().toISOString()};error='';storageWarning='';persist();
}
...
function render(focus=false){
 ...
 root.innerHTML=`<div class="layout"><aside class="steps"><p class="eyebrow">${esc(state.name)}</p><p class="session-label">${state.demo?'Démonstration':'Séance '+esc(state.code)}</p><label class="number" for="progress">Étape ${state.position+1} sur ${pages.length}</label><progress id="progress" value="${state.position+1}" max="${pages.length}"></progress><details class="mobile-sections"><summary>Changer de partie</summary>${sectionNav(p.stage)}</details><div class="desktop-sections">${sectionNav(p.stage)}</div><div class="aside-actions"><button class="small secondary" id="review">Relire mon parcours</button><button class="small secondary" id="backup">Télécharger une copie</button><button class="small text-button" id="leave">Revenir à l’accueil</button></div></aside><section class="work-area">${saveBar()}<div class="card question-card">${currentHtml(p)}<div id="limit-error" role="alert"></div><div class="actions navigation-actions"><button class="secondary" id="prev" ${state.position===0?'disabled':''}>Précédent</button>${['question','situation'].includes(p.type)?'<button class="text-button" id="skip">Passer pour le moment</button>':''}<button id="next">${state.position===pages.length-1?'Relire et terminer':'Continuer'}</button></div></div></section></div>`;
...
function reviewScreen(){
 ...
 root.innerHTML=`<div class="narrow"><p class="eyebrow">Avant de terminer</p><h1 id="page-title" tabindex="-1">Relire mon parcours</h1><p>Vous pouvez revenir sur vos réponses ou terminer avec des questions laissées de côté.</p>${saveBar()}<div class="metrics review-metrics"><div class="metric"><strong>${n.questions} / 12</strong><span>questions renseignées</span></div><div class="metric"><strong>${n.challenges} / 6</strong><span>défis renseignés</span></div><div class="metric"><strong>${n.situations} / 3</strong><span>situations renseignées</span></div></div><section class="card"><h2>${missing.length?'À reprendre si vous le souhaitez':'Toutes les questions sont renseignées'}</h2>${missing.length?`<p>${missing.length} élément${missing.length>1?'s':''} sans réponse. Une réponse donnée à l’oral peut rester vide ici.</p><div class="review-links">${missing.map(p=>`<button class="secondary small" data-page="${p.index}">${p.id} · ${esc(p.label)}</button>`).join('')}</div>`:'<p>Vous pouvez relire une partie avant de terminer.</p>'}<details><summary>Revenir à une partie du parcours</summary>${stages.filter((_,i)=>i!==3).map(s=>{const i=stages.indexOf(s);return `<button class="secondary small" data-page="${pages.findIndex(p=>p.stage===i)}">${s.title}</button>`;}).join(' ')}</details></section><section class="card"><h2>Mes priorités</h2><p>${state.answers.interests?.length?esc(state.answers.interests.join(' · ')):'Aucun sujet choisi pour le moment.'}</p><p>${esc(state.answers.goal||'Vous pourrez aussi expliquer votre objectif à l’animateur.')}</p><div class="actions"><button class="secondary" data-page="${pages.length-1}">Modifier mon bilan</button><button id="finish">${state.demo?'Terminer mon essai':'Terminer et transmettre'}</button></div></section></div>`;
