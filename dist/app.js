...
import {participantFile,sessionCode} from './transfer.js';
...
function welcome(){
 ...
 document.querySelector('#join').onsubmit=async e=>{
  e.preventDefault();if(!active){try{const f=new FormData(e.target);const name=f.get('name').trim();if(!name)throw Error('Indiquez un prénom ou pseudonyme.');if(previous&&!confirm('Commencer un nouveau parcours sur ce poste ? Téléchargez d’abord le résultat précédent s’il n’a pas été récupéré.'))return;start(name,sessionCode(f.get('code')),false,{id:crypto.randomUUID(),secret:'',local:true});render(true);}catch(err){document.querySelector('#join-error').textContent=err.message;}return;}
